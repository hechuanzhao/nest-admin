...
export default defineComponent({
  ...
  setup () {
    ...
    const handleLink = (item: RouteLocationMatched | AppRouterRecordRaw) => {
    ...
    // 是否是首页
    const isDashboard = (route: RouteLocationMatched | AppRouterRecordRaw): boolean => {
      const name = route && route.name as string
      if (!name) return false
      return name.trim().toLocaleLowerCase() === 'Dashboard'.toLocaleLowerCase()
    }

    const levelList = ref<Array<AppRouterRecordRaw | RouteLocationMatched>>()

    const getBreadcrumb = () => {
      const matched: Array<AppRouterRecordRaw | RouteLocationMatched> = currRoute.value.matched.filter(v => v.meta?.title)
      const first = matched[0]
      const dashboard = { path: '/dashboard', meta: { title: '首页' } } as AppRouterRecordRaw
