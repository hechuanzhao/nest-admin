  ...
  <section class="k-table-container">
    <el-table ref="elTableRef" :data="list" v-bind="$attrs" style="width: 100%;" v-loading="loading" >
      ...
      <template v-if="mode === 'config'">
        ...
        <el-table-column v-bind="column"  v-for="(column, index) in (columns as Array<any>)" :align="column.align || 'center'" :key="`${column.label}_${index}`" >
    ...
    <el-pagination
      ...
      background
      v-bind="pagination"
      @size-change="(size:any) => refreshData({ size, page: 1 })"
      @current-change="(page:any )=> refreshData({ page })"
      class="k-pagination"
    >
    </el-pagination>
  </section>
...
export default defineComponent({
  ...
  setup (props) {
    // 默认值
    const list = computed(() => props.data.list.map((v:any) => {
