...
import { login, LoginResult, UserLogin } from '../../../api/user';
    ...
    const loginEvent = () => {
        if (loginFormRef.value) {
            loginFormRef.value.validate(async (valid: boolean) => {
                    if (valid) {
                        loading.value = true
                        const res: ResultData<LoginResult> = await login(formData.value)
                        loading.value = false
                    if (res?.code === 200) {
  ...
  .form-wrapper {
    .form-title {
      ...
      .nest-logo {
        ...
        height: 40px;
        margin-right: 10px;
        background: url(@/icons/svg/logo-small.svg) no-repeat center / 100% 100% ;
      }
    }
    .action-wrapper {
      display: flex;
