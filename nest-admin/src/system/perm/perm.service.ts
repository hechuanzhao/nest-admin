...
import { RedisUtilService } from 'src/common/libs/redis/redis/redis.service';
...
export class PermService {
  ...
  async findUserPerms(userId: string): Promise<RouteDto[]> {
    ...
    if (result && JSON.parse(result).length ) return JSON.parse(result)
    // const
  ...
  async findAppAllRoutesBySwaggerApi(): Promise<RouteDto[]> {
    // 暂时这样
    const { data } = await lastValueFrom(this.http.get(`http://localhost:${this.config.get('app.port')}/api/swagger-doc-json`))
    const routes = []
    if (data?.paths) {
